...
async function main() {
  ...
  if (idxRes.rows?.length) console.table(idxRes.rows);
  ...
  else console.log("No columns found for extranet.\"RatePlan\" (unexpected).");
  ...
  if (plans.rows?.length) console.table(plans.rows);
  else console.log("No rate plans found.");
