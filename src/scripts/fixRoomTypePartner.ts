...
async function main() {
  ...
  const rt = await client.query(
    `SELECT "id","partnerId","name" FROM extranet."RoomType" WHERE "id" = $1`,
    [roomTypeId]
  );
  console.table(rt.rows);

  let partnerId: number | null = rt.rows?.[0]?.partnerId ?? null;

  if (!partnerId) {
    console.log("\nRoomType.partnerId is NULL. Looking up from RatePlan…");
    const rr = await client.query(
      `SELECT MIN("partnerId") AS partner_id
    ...
    if (partnerId) {
      const upd = await client.query(
        `UPDATE extranet."RoomType" SET "partnerId" = $1 WHERE "id" = $2`,
