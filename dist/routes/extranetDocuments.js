...
const ALLOWED_STATUSES = ["REQUIRED", "SUBMITTED", "APPROVED", "REJECTED"];
function toDocType(v) {
    const s = String(v ?? "").toUpperCase();
    return ALLOWED_TYPES.includes(s) ? s : null;
}
function toDocStatus(v) {
    const s = String(v ?? "").toUpperCase();
...
router.use((req, res, next) => {
    const pid = res.locals?.partner?.id ??
    ...
    if (!pid || Number.isNaN(Number(pid)))
        return res.status(401).json({ error: "unauthorized_no_partner" });
    ...
    try {
        ...
        if (e?.code === "P2003") {
            try {
                ...
                const row = await prisma.propertyDocument.create({
                    data: {
                        partnerId: profile.id, // satisfies wrong FK target if DB expects PropertyProfile(id)
                        type: normalizedType,
                        key,
                        url,
                        fileName: fileName ?? null,
        ...
        if (e?.code === "P2002")
            payload.conflict = e?.meta?.target ?? null;
...
router.put("/:id", async (req, res) => {
    ...
    if (type !== undefined) {
        ...
        if (!t)
            return res.status(400).json({ error: "invalid_type", allowed: ALLOWED_TYPES });
        normalizedType = t;
    }
