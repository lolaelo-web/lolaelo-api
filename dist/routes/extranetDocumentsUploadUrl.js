...
const ALLOWED = (process.env.DOCS_ALLOWED_MIME ??
    "application/pdf,image/jpeg,image/png,image/webp,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/zip").split(",").map(s => s.trim().toLowerCase());
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!ALLOWED.includes(ct))
            return res.status(400).json({ error: "Unsupported contentType", allowed: ALLOWED });
        if (size > MAX_MB * 1024 * 1024)
            return res.status(400).json({ error: "File too large", maxMB: MAX_MB });
        ...
        if (!S3_BUCKET)
            return res.status(500).json({ error: "Missing S3_BUCKET" });
