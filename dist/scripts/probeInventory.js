...
async function main() {
    ...
    const roomRow = await client.query(`SELECT "partnerId","name" FROM extranet."RoomType" WHERE "id" = $1`, [roomTypeId]);
  ...
  `);
    ...
    try {
        ...
        for (const d of dates) {
            ...
            try {
                await client.query(`INSERT INTO extranet."RoomInventory"
           ...
           DO UPDATE SET "roomsOpen" = EXCLUDED."roomsOpen",
                         ...
                         "updatedAt" = NOW()`, [partnerId, roomTypeId, d, roomsOpen, minStay, isClosed]);
                upserted++;
            }
            catch (e) {
    ...
    // Read back rows
    const rows = await client.query(`SELECT "id","partnerId","roomTypeId","date","roomsOpen","minStay","isClosed","createdAt","updatedAt"
       FROM extranet."RoomInventory"
      WHERE "roomTypeId" = $1 AND "date" BETWEEN $2 AND $3
      ORDER BY "date" ASC`, [roomTypeId, dates[0], dates[dates.length - 1]]);
